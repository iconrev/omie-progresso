...
class UpdateFiles:
    ...
    def __init__(self, source: str) -> None:
        ...
        self.__path_valids = ["base", "externo", "interno", "metas"]
    ...
    def __update_eslint(self, dir):
        print(f"[INFO] Atualizando .eslintrc.js")

        path_handler = ".eslintrc.js"

        full_path_source = (
            self.__full_path_from_root + "/" + self.path_name_base + "/" + path_handler
        )
        full_path_file_destination = (
            f"{self.__full_path_from_root}/{dir}/" + path_handler
        )
        try:
            shutil.copyfile(full_path_source, full_path_file_destination)
        except Exception as e:
            print(f"[ERRO] {e}")

    def __update_handler(self, dir):
        print(f"[INFO] Atualizando handler.js")

        path_handler = "services/handler.js"
    ...
    def run(self) -> None:
        ...
        for dir in folders:
            ...
            self.__update_dir("models", dir)
            self.__update_dir("migrations", dir)
            self.__update_dir("auth", dir)
            self.__update_dir("seeders", dir)
            self.__update_handler(dir)
            self.__update_eslint(dir)
