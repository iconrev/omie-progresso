...
class TableAvaliacao extends Component {
	...
	submit = async (event) => {
		...
		for (let i = 0; i < this.state.data.length; i++) {
			let items = this.state.data[i].itens;
			for (let j = 0; j < items.length; j++) {
				data[items[j].id] = items[j].avaliacao;
			}
		}
		...
		const { resource, ano } = this.props;

		const response =
			await AmbienteInternoService.updateResourceInternoAvaliacao(
				resource,
				ano,
				data
			);
	...
	handleMenu = (event) => {
		...
		this.props.history.push(
			`/hub/${this.props.companyId}/gestao/categoria/diagnostico/interno`
		);
	};
