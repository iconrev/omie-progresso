...
class ClienteForm extends Component {
	...
	loadData = async () => {
		const response = await AmbienteExternoService.getListaResource("clientes");
	...
	getSelectedValue = (value, reverse = false) => {
		if (reverse) {
			// do nothing...
			let pos = this.clienteSelectData.indexOf(value);
			if (pos === -1) {
				return 0;
			}
			return pos;
			...
			return this.clienteSelectData[index];
		}
	};
	...
	handleSubmit = async (event) => {
		...
		for (const cliente of this.state.clientes) {
			list.push({
				id: cliente.id,
	...
	calcularPontos = async (resource) => {
		...
		for (const item of this.state.clientes) {
			if (item[resource] === "Eu atendo") {
				pontos.push(10);
			} else if (item[resource] === "Eu encanto") {
				pontos.push(POINT_MAX_ITEM);
			} else if (item[resource] === "Eu decepciono") {
				pontos.push(0);
			}
		}
	...
	loaded = () => {
		return (
			<div className="animated fadeIn">
				{this.avaliacaoClientes()}
				<div>
					<SwotForm swot={"clientes"} />
				</div>
			</div>
		);
	};
