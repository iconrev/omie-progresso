...
module.exports.concorrentes_avaliacao_grafico = async (event, action) => {
  ...
  try {
    ...
    for (const ano of anos) {
      let concorrentesYear = await CarregarDados.getAll(companyId, Concorrentes, ano)
      let avaliacaoYear = await _avaliar(concorrentesYear)
      if (avaliacaoYear['status'] === 'success') {
        await UpdateDadosAmbienteExterno.UpdateMedia(avaliacaoYear, companyId, ano, 'concorrentes')
      }
      avaliacao[ano] = {
