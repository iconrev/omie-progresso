...
module.exports.clientes_avaliacao = async (event, action) => {
  ...
  try {
    ...
    for (const ano of anos) {
      let clientesYear = await CarregarDados.getAll(companyId, Clientes, ano)
      let avaliacaoYear = await _avaliar(clientesYear)
      if (avaliacaoYear['status'] === 'success') {
        await UpdateDadosAmbienteExterno.UpdateMedia(avaliacaoYear, companyId, ano, 'clientes')
      }
      avaliacao[ano] = {
